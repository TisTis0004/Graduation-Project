...
class SyntheticEMGInjection(nn.Module):
    ...
    never seen during training. EMG noise is NOT Gaussian ΓÇö it's:
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for b in range(B):
            ...
            kernel_size = max(3, self.fs // 10)  # ~25 samples at 256Hz Γëê removes <10Hz
            if kernel_size % 2 == 0:
                kernel_size += 1
            ...
            # 4. Create burst mask ΓÇö EMG is bursty, not continuous
...
class EEGToSpectrogram(nn.Module):
    def __init__(self, n_fft=256, hop_length=32, max_freq_bin=40):
        ...
        # NEW: EEG-specific augmentations ΓÇö LIGHTENED for from-scratch training
        ...
        self.amplitude_rescale = RandomAmplitudeRescale(
            scale_range=(0.85, 1.2),           # Very tight ΓÇö just mild gain jitter
            prob=0.3,
        )
