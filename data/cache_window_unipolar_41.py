...
# TODO: Fill in the EXACT 41 channels that the best_model_checkpoint.pt was trained on!
# Without this, it will fail to match the 41 channels the weights expect.
CANONICAL_41 = [
    # "fp1", "fp2", ... 
]
...
class CacheConfig:
    ...
    # 9-class Seizure Dataset Labels
...
# =========================================================
# LABELS (MULTICLASS: 9 CLASSES)
# =========================================================

def build_9class_label_vocab():
...
def extract_unipolar_channels(
    ...
    if len(target_channels) == 0:
        raise ValueError("CANONICAL_41 must be filled in with 41 channel names!")
...
def cache_one_record_windows(
    ...
    x_full, montage_meta = extract_unipolar_channels(
        data=raw_data, original_channels=original_channels, target_channels=CANONICAL_41
    )
    ...
    for st in starts:
        ...
        # In multiclass, if it's not in the 9 classes, skip it or mark as bckg
        if raw_label not in label_to_id:
            raw_label = "bckg"

        raw_label_counts[raw_label] += 1
        final_label_counts[raw_label] += 1

        xs.append(torch.from_numpy(xw))
        ys.append(label_to_id[raw_label])
    ...
    torch.save({
        ...
        "meta": {
            ...
            "label_mode": "multiclass_9", "bad_windows_skipped": bad_windows,
            **montage_meta,
        },
    }, out_path)
...
def build_cache_from_json(cfg: CacheConfig) -> Path:
    ...
    label_to_id, id_to_label = build_9class_label_vocab()
...
if __name__ == "__main__":
    cfg = CacheConfig(
        ...
        out_dir=r"cache_windows_unipolar_41_multiclass_eval",
        fs=256, # Assuming the 41-channel model uses 256Hz (2560 timepoints). If it uses 250, change this!
        window_sec=10,
        stride_sec=5,
        l_freq=0.5,
        h_freq=40.0,
