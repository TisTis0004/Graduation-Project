...
WHY this ensemble works so well:
  - The 2D model excels at spectral pattern recognition (frequency ├ù time)
...
# =========================================================
# CONFIG ΓÇö Define your ensemble members
# =========================================================
...
def predict_eegnet_model(model, pt_path, device, batch_size=64):
    ...
    for start in range(0, n, batch_size):
        ...
        x_batch = x_all[start:end].to(device)
        # No extra normalization ΓÇö data is already Median+IQR normalized from cache
...
def main():
    ...
    for cfg in SPECTROGRAM_MODELS:
        ...
        if not Path(cfg["checkpoint"]).exists():
            print(f"  ΓÜá∩╕Å SKIPPED ΓÇö checkpoint not found")
            continue
    ...
    if not spec_models and not eegnet_models:
        print("Γ¥î No models loaded! Check checkpoint paths.")
        return
    ...
    spec_transform.eval()
    # No extra normalization needed ΓÇö cached data is already Median+IQR normalized
    ...
    # ===========================================================
    # PHASE 3: Ensemble ΓÇö Average by model group
    # ===========================================================
    print(f"\n{'='*70}")
    ...
    # ===========================================================
    # PHASE 5: Full pipeline ΓÇö best ensemble + voting + min-duration
    # ===========================================================
    print(f"\n{'='*70}")
    ...
    print(f"\n{'='*70}")
    print(f"≡ƒÅå BEST STRATEGY: {best_name}")
